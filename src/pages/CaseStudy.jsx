...
function CaseStudy() {
  ...
  const scenarioContent = (
    <>
      {challenge.scenario && (
        <div>
          <h2 className="text-base sm:text-lg font-bold text-white mb-2">{challenge.scenario.title}</h2>
  ...
  return (
    <div className="h-[calc(100dvh-73px)] flex flex-col">
      {/* Top navigation */}
      <div className="flex items-center justify-between px-3 sm:px-4 py-1.5 sm:py-2 border-b border-slate-700 bg-slate-900/50 flex-shrink-0">
        <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
          ...
          <div className="min-w-0">
            <h1 className="text-white font-medium text-sm truncate flex items-center gap-2">
              <span className="text-slate-500 text-xs">{world?.emoji} {quest.name}</span>
          ...
          {sectionTitle && (
            <Link
              ...
              className="hidden md:flex items-center gap-1 px-2 py-1 bg-brand-primary/10 border border-brand-primary/30 text-brand-primary text-xs rounded hover:bg-brand-primary/20 transition-colors flex-shrink-0"
            >
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          {prevChallenge && (
            <button
              ...
              className="flex items-center gap-1 px-2 py-1.5 text-slate-400 hover:text-white text-xs transition-colors"
            >
              <ChevronLeft className="w-3.5 h-3.5" /> <span className="hidden sm:inline">{t('case:caseStudy.prevQuestion')}</span>
            </button>
          )}
          {nextChallenge && (
            <button
              ...
              className="flex items-center gap-1 px-2 py-1.5 text-slate-400 hover:text-white text-xs transition-colors"
            >
              <span className="hidden sm:inline">{t('case:caseStudy.nextQuestion')}</span> <ChevronRight className="w-3.5 h-3.5" />
            </button>
          )}
        </div>
      </div>
      ...
      <div className="flex-1 hidden md:flex min-h-0 overflow-hidden">
        {/* Left: Scenario */}
        <div className="flex-1 overflow-y-auto p-4 lg:p-5 border-r border-slate-700">
          {scenarioContent}
        </div>
        {/* Right: Question + Answer */}
        <div className="w-[420px] lg:w-[500px] xl:w-[540px] flex flex-col overflow-y-auto">
      ...
      {/* Bottom: Hint area */}
      <div className="border-t border-slate-700 bg-slate-900 px-3 sm:px-4 py-2 sm:py-2.5 pr-14 sm:pr-16 flex-shrink-0">
        <div className="flex items-center gap-2 sm:gap-3 max-w-full">
          <div className="hidden sm:flex w-10 h-10 rounded-full bg-brand-accent/20 items-center justify-center flex-shrink-0 overflow-hidden">
            <span className="text-brand-accent font-bold text-[10px] leading-tight text-center">{t('case:caseStudy.xiaoDi')}</span>
          </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-3 mt-1">
                  {hintLevel < (challenge.hints?.length || 3) && (
                    <button onClick={getHint} className="text-brand-accent text-xs hover:underline">
          ...
          <div className="flex items-center gap-1.5 flex-shrink-0">
            <button
              ...
              className="flex items-center gap-1 sm:gap-1.5 px-2.5 sm:px-3 py-1.5 sm:py-2 bg-slate-800 text-brand-accent rounded-lg hover:bg-slate-700 transition-colors disabled:opacity-50 text-xs"
            >
            ...
            {submitted && nextChallenge && (
              <button
                onClick={() => navigate(`/di-quest/case/${worldId}/${questId}/${nextChallenge.id}`)}
                className="flex items-center gap-1 px-3 sm:px-4 py-1.5 sm:py-2 bg-brand-primary text-white rounded-lg hover:bg-blue-600 transition-colors text-xs"
              >
                <span>{t('case:caseStudy.nextQuestion')}</span>
            ...
            {submitted && !nextChallenge && nextQuestInWorld && (
              <button
                ...
                className="flex items-center gap-1 px-3 sm:px-4 py-1.5 sm:py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 transition-colors text-xs"
              >
